...
def render_report(generous: dict, tight: dict) -> str:
    ...
    parts.append("")
    parts.append(
        f"*Real runs of `examples/token_budget/main.py` against `{model}` at `{endpoint}`.*"
    )
    parts.append("")
    ...
    parts.append(
        textwrap.dedent(
            ...
            `token_reporter` you wired onto `LangChainChiefRuntime`,
            `LangChainReasoner`, and `make_auto_execute_fn` (the
            substrate composition replaces the pre-J1
            `LangChainReasoner(..., token_reporter=...)` / chief-runtime wiring).
