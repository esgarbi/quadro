...
from quadro_langchain import (  # noqa: E402
...
LLM_BUDGET = _env_int("LLM_BUDGET", 9000, minimum=1)
...
def _classify_stage_config(stage_mode: str, *, token_reporter=None) -> dict[str, object]:
    ...
    :func:`quadro_langchain.make_auto_execute_fn` — the substrate-mode
    replacement for the pre-J1 LangChain-pipeline auto-execute-fn
    hook. The native path supplies ``supervisor=`` so the registered
    ...
    if stage_mode == "compat":
        ...
        return {"execute_fn": execute_fn}
    raise ValueError(f"Unknown stage mode: {stage_mode}")


def build_pipeline_builder(runtime: QuadroRuntime, *, stage_mode: str) -> Pipeline:
    token_reporter = runtime.meters.report_llm_tokens
    classify_cfg = _classify_stage_config(stage_mode, token_reporter=token_reporter)
    return (
