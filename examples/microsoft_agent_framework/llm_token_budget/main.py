...
the Microsoft Agent Framework tags each one (`urgency`, `category`,
`suggested_reply`), and the whole run is governed by an ``AllOf``
composition of three sponsors:
...
Token usage flows into the sponsor via the MAF adapter's new
``token_reporter`` hook, which extracts ``usage`` from every MAF turn
and pushes the sum into the runtime's shared ``MeterBundle``.
"""
...
# MAF spins up short-lived asyncio loops per worker turn. When the outer
# RunLoop finalises, ``httpx.AsyncClient.aclose()`` futures from spent
# loops surface as "Event loop is closed" — harmless, but noisy. Suppress.
...
class TicketTag(BaseModel):
    """Schema the classifier stage must return. MAF enforces it via
    ``response_format`` and the stage marks the task ``classify_failed``
    on a schema-validation miss."""
...
def build_runtime_and_pipeline():
    ...
    pipeline = (
        ...
        .llm(
            ...
            # THE line that makes LlmTokenBudgetSponsor work: every MAF
            # turn (chief + classifier) now reports its usage into the
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "When omitted, nothing is written to disk and the script just "
            "prints the terminal summary."
        ),
    )
