...
Simple API:
    ...
    ArticleProducer(bc, chief, target=10, batch_size=2, batch_interval_minutes=5.0)

    # Explicit choreography: list of (batch_size, wait_minutes_before_posting)
    ArticleProducer(bc, chief, target=8, choreography=[
...
class ArticleProducer:
    ...
    Args:
        ...
        chief:                  ChiefAgent — nudged after each batch.
    ...
    def _loop(self) -> None:
        for batch_size, wait_minutes in self._steps:
            ...
            if posted_this_batch > 0:
                try:
                    self._chief.nudge(trigger="seed")
                except Exception as exc:
                    logger.debug("ArticleProducer: chief nudge failed: %s", exc)
