...
This example demonstrates Quadro's **continuity story** without any LLM
framework. It uses Quadro's deterministic chief and pure Python workers; adding
an LLM adapter would mean composing `Pipeline(...)` with a `Reasoner` and
framework runtime as shown in `examples/minimal/` or the LLM-backed examples.
...
workflow path even though Quadro's saga DSL would also work here.
The teaching purpose is the **CRM-themed sponsor pattern** — using
domain-specific sponsors to govern run-loop decisions. Converting
to a saga would mix that with declarative-authoring concerns and
obscure the sponsor as the centre of the example.
