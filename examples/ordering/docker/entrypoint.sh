#!/usr/bin/env sh
# entrypoint.sh — start the Board UI in the background, then run the ordering system.
#
# Process model:
...
#   3. Remove any stale orders.db so each container start is a fresh run
#   4. Start Board UI in the background (python -m quadro.ui orders.db)
#   5. Foreground main_pipeline.py — when it exits the container stops
#
# Environment variables (set in docker-compose.yml or .env):
#   LLM_PROVIDER             "ollama" or "openai"          (default "ollama")
#   ORDERING_TARGET          number of orders to ship       (default 10)
#   ORDERING_CYCLES          max chief decision cycles      (default 1000)
#   ORDERING_PROFILE         emission profile               (default "steady")
#   ORDERING_CHOREOGRAPHY    named choreography or empty    (default "")
...
#   UI_PORT                  board UI port                  (default 8081)
...
DB_PATH="/app/examples/ordering/orders.db"
if [ -f "${DB_PATH}" ]; then
    echo "[entrypoint] Removing stale orders.db"
    rm -f "${DB_PATH}"
fi
...
# ── 4. Build main_pipeline.py arguments and run ──────────────────────────────
...
echo "[entrypoint] Starting ordering system: python main_pipeline.py ${ARGS}"
cd /app/examples/ordering

# Use exec so main_pipeline.py replaces this shell and receives signals cleanly.
