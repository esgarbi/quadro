...
_PROFILES: dict[str, dict] = {
    "agressive": {
        "batch_sizes": ([10, 15, 20, 30], [25, 35, 25, 15]),
        "delay_mean": 1.0,
        "delay_min": 0.05,
        "delay_max": 0.5,
        "dropout_prob": 0.0,
        "description": "Extremely aggressive bursts — floods Chief with maximum orders as fast as possible, minimal delay.",
    },
    "burst": {
    ...
    def _production_loop(self) -> None:
        while self.running:
            ...
            if n > 0:
                ...
                for order in orders:
                    try:
                        label = f"{order['customer_name']} — {order['product_name']} x{order['quantity']}"
                        self._bc.post_task("order", label, notes=[order])
                        posted += 1
                    except Exception as exc:  # noqa: BLE001
                        logger.warning("OrderProducer: post_task failed: %s", exc)

                if posted > 0:
                    self._emission_count += 1
                    self._order_count += posted
                    logger.debug(
                        "OrderProducer: posted %d order(s) [profile=%s, total=%d]",
                        posted,
                        self._profile_name,
                        self._order_count,
                    )
