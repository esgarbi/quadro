...
#   - Launches main_pipeline.py under debugpy, waiting for the IDE to attach
#   - Set DEBUGPY_WAIT=0 to skip waiting (debugpy still listens)
#
# Attach from VS Code / Cursor with the "Attach: Newsroom Docker" launch config.
...
# ── 4. Launch main_pipeline.py under debugpy ─────────────────────────────────
