...
**ArticleProducer** that decouples task creation from task routing. Its default
LLM adapter is MAF (`quadro_maf`): the pipeline composes `Pipeline(...)` with
...
graph TD
    ...
    subgraph Infra
        ...
        RL[RunLoop<br/>GoalSponsor: published ≥ target]
    end
...
```
newsroom/
├── main_pipeline.py     Entry point — pipeline-DSL wiring with sagas, chief, producer, run loop
├── sagas/               Saga modules — one per stage
│   ├── __init__.py
│   ├── _common.py       Shared deterministic helpers (PubMed fetch, persistence, telemetry)
│   ├── ideation.py      Ideation saga (headline + brief)
│   ├── research.py      Research saga (PubMed strategy + citations)
│   ├── writing.py       Writing saga (long-form draft)
│   └── review.py        Review saga (approve / request revision)
├── tools.py             Chief board tools (advance_to_*, discard_task)
├── schemas.py           Pydantic models (ArticleBrief, ResearchOutput, Headline, etc.)
├── producer.py          ArticleProducer — background task feeder
├── runtime.py           Runtime construction (board, sponsor, lifecycle, choreography)
├── docker/              Docker setup — see Running with Docker below
├── prompts/
│   ├── chief.md         Managing editor system prompt
│   ├── headline.md      Headline generation (first reason step in ideation saga)
│   ├── ideation.md      Article brief generation (second reason step)
...
The four pipeline stages (ideation, research, writing, review) are
implemented as **sagas** — declarative chains of named steps with
per-step persistence and telemetry. See [`sagas/`](sagas/) for the
saga definitions and the deterministic helpers (PubMed fetch, token
attribution, disk persistence) under
[`sagas/_common.py`](sagas/_common.py). The earlier `main.py` (manual
`WorkerPool + ChiefAgent` wiring) was retired in favour of
`main_pipeline.py`, which uses Quadro's `Pipeline` builder DSL
throughout.
...
python examples/newsroom/main_pipeline.py --target 1

# Larger run with more cycles
python examples/newsroom/main_pipeline.py --target 10 --cycles 1000

# Named choreography (batched article posting)
python examples/newsroom/main_pipeline.py --choreography sleep_study
```
...
```bash
rm -f examples/newsroom/newsroom.db
rm -rf examples/newsroom/output/
```
...
`src/quadro/` and `examples/newsroom/` both work without rebuilding.
