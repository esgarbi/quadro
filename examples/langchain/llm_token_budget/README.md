...
```mermaid
flowchart LR
    Worker["classify execute_fn<br/>(auto-generated by LangChainPipeline)"] -->|ChatOpenAI.ainvoke| LLM["OpenAI-compat /v1"]
...
2. `LangChainPipeline.llm(token_reporter=runtime.meters.report_llm_tokens)`
   tells the LangChain adapter to extract token usage from every call
   (chief and classifier) and push `prompt_tokens + completion_tokens`
   into that bundle.
