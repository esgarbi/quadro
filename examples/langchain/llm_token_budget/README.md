...
By default, this example now uses the **native LangChain runtime path**
(`stage(supervisor=...)`), which exercises the same control-plane runtime
delegation model as the MAF sibling examples.
...
2. `LangChainPipeline.llm(token_reporter=runtime.meters.report_llm_tokens)`
   tells the LangChain integration to extract token usage from every
   call (chief + classifier stage, across both native and compat modes)
   and push `prompt_tokens + completion_tokens` into that bundle.
