"""
LangChain/LangGraph runtime plugin for native graph/supervisor stages.

This plugin keeps Quadro as the governance control plane while delegating
graph/supervisor execution details to LangChain/LangGraph and returning
normalized stage/chief results back to Pipeline.
"""
...
@dataclass
class LangChainRuntime(FrameworkRuntime):
    """Framework runtime plugin for ``stage(graph=...)`` and ``stage(supervisor=...)``."""

    client_factory_getter: Any
    chief_name_prefix_getter: Any | None = None
    token_reporter_getter: Any | None = None
    runtime_id: str = "langchain_native"
    ...
    def _resolve_client_factory(self):  # noqa: ANN202
        ...
        if factory is None:
            raise RuntimeError("LangChainRuntime requires a client_factory")
        return factory
    ...
    def decorate_tools(self, descriptors: list[Any]) -> list:
        from ..integrations.langchain import _decorate_descriptors
    ...
    async def run_chief_turn(
        ...
        *,
        ...
        from ..integrations.langchain import _run_chief_workflow
    ...
    async def run_stage(self, ctx: RuntimeContext) -> StageRunResult:
        ...
        if selected is None:
            raise ValueError("LangChain runtime plugin received a stage without graph/supervisor")
