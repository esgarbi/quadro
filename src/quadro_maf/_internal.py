"""
Internal helpers shared by :mod:`quadro_maf.reasoner` and
:mod:`quadro_maf.runtime`.

The public surface is re-exported from :mod:`quadro_maf`; nothing here
is part of the documented API except where explicitly named (e.g.
:func:`configure`, :func:`llm_call`, :func:`tools_from_lifecycle`,
:func:`make_auto_execute_fn`, and :class:`MafStageSpec`).
"""
...
#
# The try/except preserves the import-time zero-dependency guarantee of this
# adapter package: ``quadro_maf`` stays importable even without
# agent-framework installed (useful for IDE tooling and for tests that
# only exercise pure-Python helpers). Any actual use goes through
# :func:`_ensure_maf` which raises a friendly error in that case.
try:
...
_MAF_IMPORT_ERROR: str = (
    ...
    "Install it with:  pip install 'quadro[maf]'"
)
...
def _default_client_factory():
    ...
    if not key:
        raise RuntimeError(
            "OPENAI_API_KEY not set. Either call quadro_maf.configure() "
...
def make_auto_execute_fn(
    ...
    *,
    ...
    """Build an ``execute_fn`` for a MAF prompt-in / schema-out stage.

    The helper replaces the deleted ``MafPipeline._make_auto_execute_fn``
    hook. Users now compose execution explicitly: wire the helper into
    ``Pipeline.stage(..., execute_fn=make_auto_execute_fn(...))``.

    Two calling conventions are supported:

    1. Pass a pre-built :class:`MafStageSpec` as the first positional
       argument plus ``client_factory``/``token_reporter``.
    2. Pass every field as a keyword argument. This is usually more
       readable at call sites that were previously written as
       ``.stage("classify", prompt=..., output_schema=..., ...)``.

    A ``client_factory`` is required either way.
    """
    if spec is not None:
        if not isinstance(spec, MafStageSpec):
            raise TypeError(
                "make_auto_execute_fn(spec, ...) requires a MafStageSpec "
    ...
    if client_factory is None:
        raise ValueError(
            ...
            "callable returning an OpenAIChatClient). Pass the same "
            "factory you use for MafReasoner / MafChiefRuntime."
        )
